...
document.addEventListener('touchstart', ev => {
    if (ev.touches[ev.touches.length-1].clientY<=ev.touches[ev.touches.length-1].clientX*prop) {
        if (ev.touches[ev.touches.length-1].clientY<=(canv.width - ev.touches[ev.touches.length-1].clientX)*prop) {addMove('up')}
        else {addMove('right')}
    } else {
        if (ev.touches[ev.touches.length-1].clientY<=(canv.width - ev.touches[ev.touches.length-1].clientX)*prop) {addMove('left')}
        else {addMove('down')}
    }
})
